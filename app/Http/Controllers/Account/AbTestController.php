...
use App\Exports\ResponsesABExport;
...
class AbTestController extends Controller {
    public function index(Request $request) {
        ...
        if (auth()->user()->hasPermissionTo('ab.index.full')) {
        ...
        return redirect()->route('account.ab.id', ['id' => $lowestTitleId]);
    }
    ...
    public function show(Request $request, $id) {
        ...
        if (!auth()->user()->hasPermissionTo('ab.index.full') && $survey->user_id != $userID) {
        ...
        $responses = Cache::remember('responses-ab-' . $id, $cacheExpiredMinutes, function () use ($id) {
            return SurveyResponses::where('survey_id', $id)
                ->where('response_data', 'LIKE', '%"tam"%')
                ->get();
        });

        // Mulai - Format data AB - responsesFormated
        ...
        // Akhir - Format data AB - responsesFormated
    ...
    private function countRespondents($surveyId, $responsesFormated) {
        $totalResponsesWithAB = SurveyResponses::where('survey_id', $surveyId)
            ->where('response_data', 'LIKE', '%"ab"%')
            ->count();

        return $totalResponsesWithAB;
    }
    ...
    private function getAbResults($responses) {
        if ($responses->isEmpty()) {
            return [];
        }
        ...
        foreach ($responses as $response) {
            ...
            foreach ($responseData['ab_testing'] as $comparison) {
                ...
                foreach ($comparison['responses'] as $response) {
                    $id = $response['id'];
                    $selected = $response['selected'];

                    if (!isset($results[$name][$id])) {
                        $results[$name][$id] = [
                            'a' => 0,
                            'b' => 0,
                            'total' => 0,
                    ...
                    $results[$name][$id][$selected]++;
                    $results[$name][$id]['total']++;

                    if (isset($response['reason']) && !empty($response['reason'])) {
                        $results[$name][$id]['reasons_'.$selected][] = $response['reason'];
                    }
                }
            }
        }
        ...
        // percentage
        foreach ($results as $name => $comparisons) {
            foreach ($comparisons as $id => $data) {
                if ($data['total'] > 0) {
                    $results[$name][$id]['a_percentage'] = round(($data['a'] / $data ['total']) * 100, 2);
                    $results[$name][$id]['b_percentage'] = round(($data['b'] / $data ['total']) * 100, 2);
                }
            }
        }
    ...
    private function getAbTestSurveyResults($responses) {
        $abTestSurveyResults = [];
    ...
    public function export($survey_id) {
        ...
        return Excel::download(new ResponsesABExport($survey_id), $fileName);
    }
}
